...
async function sendLocationToServer(p: Geolocation.GeoPosition) {
  ...
  if (!token) { console.log('[BG] 토큰 없음'); return; }
  const { latitude, longitude } = p.coords;
  console.log(`[BG] 전송 시도: ${latitude}, ${longitude}`);
  try {
    await axios.post(`${BASE_URL}/api/location`, {
      latitude, longitude, recordedAt: new Date(p.timestamp).toISOString(),
    }, { headers: { Authorization: `Bearer ${token}` }});
    console.log('[BG] 전송 성공');
  } catch (e:any) {
    console.log('[BG] 전송 실패:', e?.message ?? e);
  }
}
...
export async function initBackgroundLocation() {
  ...
  if (watchId !== null) { console.log('[BG] 이미 실행 중'); return; }

  console.log('[BG] watchPosition 등록 시도');
  watchId = Geolocation.watchPosition(
    (pos) => { console.log('[BG] 위치 업데이트:', pos.coords); sendLocationToServer(pos); },
    (err) => { console.log('[BG] watchPosition 에러:', err); },
    {
